...
def load_model():
    ...
    socket.emit("progress",{"data":"Model Loading..."})
    ...
    socket.emit("progress",{"data":"All Models Loaded"})
...
def serve_video(filename):
    return send_file(os.path.join(ANNOTATED_FOLDER, filename))
...
def predict_img(data):
    ...
    socket.emit("progress",{"data":"Initiating Image Prediction"})

    file_path = os.path.join(UPLOAD_FOLDER, data.filename)
    data.save(file_path)

    print("Image Uploaded!")
    socket.emit("progress",{"data":"Image Uploaded"})

    model = cache.get('model')

    results = model(file_path)
    print("Model Running!")
    socket.emit("progress",{"data":"Model Running!"})
    ...
    socket.emit("progress",{"data":"Annotated Image Saved!"})
...
def predict_video(data):
    ...
    # font = cache.get('font')
    socket.emit("progress",{"data":"YOLO LOADED"})
    ...
    if file_extension == 'mp4':
        ...
        socket.emit("progress",{"data":"mp4 Video Detected"})
        ...
        socket.emit("progress",{"data":"Starting Video Processing"})
...
def predict_DeepFake(data):
    ...
    socket.emit("progress",{"data":"DeepFake Detection Initiated"})
    ...
    print("File Uploaded")
    socket.emit("progress",{"data":"File Uploaded"})

    img=Image.open(upload_path)
    face=mtcnn(img)
    
    print("Face detected")
    socket.emit("progress",{"data":"Face Detected"})

    if face is None:
        return ApiError("No Face Detected!",HTTP_400_BAD_REQUEST)
    face=face.unsqueeze(0) 
    print("add the batch dimension")
    socket.emit("progress",{"data":"Add the batch Dimension"})

    face =F.interpolate(face,size=(256,256),mode='bilinear',align_corners=False)

    print("convert the face into a numpy array to be able to plot it")
    socket.emit("progress",{"data":"convert the face into a numpy array to be able to plot it"})
    ...
    socket.emit("progress",{"data":"Face Dimensions Setup"})
    ...
    socket.emit("progress",{"data":"Targets Identified"})
    ...
    socket.emit("progress",{"data":"DeepFake Prediction Begin"})
    ...
    socket.emit("progress",{"data":"Prediction Saving"})
...
def dashboard():
    ...
    if 'predict_img' in request.args:
        ...
        img=request.files['img']
        try:
            annotated_url=predict_video(img)
            socket.emit("progress",{"data":"Processed video is downloaded"})
            data['resultImage'] = "Video has been downloaded"
            data['url']=annotated_url
            data['type']="video"
        except Exception as e:
            data['message'] = str(e)
    ...
    return ApiResponse("Dashboard Working!",HTTP_200_OK,data)
